...
# Plotting Tb Along wire target
z = 0
tbsum=[]                   # Produce lists to append
zaxis=[]
while z < 1000:                   # Loop to sum through all temperature values across each plane of z in steps of 1um 
    tbs = (np.sum(tb[:,:,z]))     # from 0um to 1000um
    #length= len(tb[:,:,z])
    #print(length)
    #print(tbs)
    #print(z)
    tbsum.append(tbs/(nx*ny))     # Average as suming all values and then dividing by total amount of data points 
    zaxis.append(z)                # for each (x,y) plane at specific z location
    z= z+1
plt.plot(zaxis,tbsum)
plt.xlabel(r'z ($\mu$m)')
plt.ylabel(r'Temp(eV)')
plt.show()